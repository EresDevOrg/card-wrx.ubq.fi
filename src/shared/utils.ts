...
export async function verifyOtp(otp: string, session: Session, smsResponse: SmsOtpResponse): Promise<VerifyOtpResponse | null> {
  ...
  const smsVerifyData: VerifyOtpResponse = await responseVerify.json();
